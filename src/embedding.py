...
def _process_single_part_threaded(args):
    """Worker function for multi-threading - processes a single video part"""
    from decord import bridge
    # Ensure decord bridge is set in this thread
    bridge.set_bridge("torch")
    
    i, part_path, all_tags, start_time, verbose, fast = args
    
    if verbose > 0:
        print(f'Processing file {part_path}: Start Time = {start_time}')
    
    try:
        if fast:
            st_time = time.time()
            vid_pix_i, y_i, labelsdict, end_time = getipseglevel_fast(
                part_path, all_tags,
                start_time=start_time,
                clip_duration=2,
                clips_per_video=20,
            ...
            st_time = time.time()
            vid_pix_i, y_i, labelsdict, end_time = getipseglevel_mod(
                part_path, all_tags,
                start_time=start_time,
                clip_duration=2,
                clips_per_video=20,
        ...
        if verbose > 0:
            print(f'Following Tags exist = {y_i}')
        
        # Ensure it's a tensor
        if not isinstance(vid_pix_i, torch.Tensor):
            vid_pix_i = torch.from_numpy(vid_pix_i)
        
        return (vid_pix_i, y_i, labelsdict, end_time, i)
...
def getembedpartpaths_mod_parallel(vidfilepaths, all_tags, model=None, verbose=0, fast=False, num_workers=4):
    """
    Process video parts in parallel using multi-threading.
    
    Args:
        num_workers: Number of parallel threads to use (default: 4)
    """
    all_embed = []
    all_labels = []
    all_labels_dict = []
    ...
    partdict, minp, maxp = mappartspath_mod(vidfilepaths)
    if verbose:
        print(f'Parts: Min = {minp}, Max = {maxp}, partdict = {partdict}')
    ...
    for i in range(maxp):
        if i in partdict:
            part_path = partdict[i]
            part_args.append((i, part_path, all_tags, start_time, verbose, fast))
    
    # Process parts in parallel using threads
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        results = list(executor.map(_process_single_part_threaded, part_args))
    
    # Filter out None results and sort by part index
    valid_results = [r for r in results if r is not None]
    valid_results.sort(key=lambda x: x[4])  # Sort by part_idx
    
    # Process results sequentially to compute embeddings on GPU
    for result in valid_results:
        vid_pix_i, y_i, labelsdict, end_time, part_idx = result
        ...
        event_inputs = {ModalityType.VISION: vid_pix_i}
        with torch.no_grad():
            event_embeddings = model(event_inputs)
        embed_np = event_embeddings['vision'].cpu().numpy()
